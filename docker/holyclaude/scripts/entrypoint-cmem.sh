...
#   2. Refresh Humanize KR plugin (best effort, non-blocking).
#   3. Hand off to the ORIGINAL base entrypoint, which performs UID/GID remap,
...
# Refresh Humanize KR plugin; never block container startup on plugin failure.
if ! /etc/holyclaude/humanize-bootstrap.sh; then
  echo "[humanize] bootstrap reported failure — continuing to base entrypoint." >&2
fi
