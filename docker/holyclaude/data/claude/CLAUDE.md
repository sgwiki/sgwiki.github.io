...
팀장은 하위 에이전트 보고와 실행 로그를 대조해 아래 7개 항목이 각각 별도 MCP 호출로 1회 이상 성공했는지 확인한다. 하나라도 누락되거나 실패하면 작업을 중단하고 commit/push하지 않는다.
...
| 7 | dataforge `fandom_episodes` | 애니메이션 에피소드(본편·0·극장판) 줄거리 교차 확인. 산문 가공·출처 미표시·직접 인용/Fandom URL/식별자(`doc_id`·`source_type=fandom_wiki`) 노출 금지. 메타데이터 필터는 `series`만 유효 |
...
- **공통 하드 커버리지(러너 강제)**: `qaset_with_rag`·`namuwiki`·`fandom_episodes`(에피소드 줄거리, 산문 가공·`series` 필터만 유효)·`dc_gallery`(유저 수요). lore/mechanics 타입은 `sg_paper`·`sg-ontology`·`sg_game_sg0_en` 추가(팀장 보고 확인).
