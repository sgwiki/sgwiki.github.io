...
- **공통 하드 커버리지(러너 강제)**: `qaset_with_rag`·`namuwiki`·`fandom_episodes`(에피소드 줄거리, 산문 가공·`series` 필터만 유효·**호출 시도만 pass**)·`dc_gallery`(유저 수요). lore/mechanics 타입은 `sg_paper`·`sg-ontology`·`sg_game_sg0_en` 추가(팀장 보고 확인).
