...
3. MCP 커버리지 7개 항목이 모두 pass인지 확인 (아래)
...
5. `dataforge` `fandom_episodes` (애니메이션 에피소드 줄거리, **산문 가공** — `series` 필터만 유효. **호출 시도만으로 pass, 결과 무관**)
6. `namuwiki` MCP
7. `sg-ontology` MCP (기존 온톨로지 SPARQL 조회 — 중복 회피용)
