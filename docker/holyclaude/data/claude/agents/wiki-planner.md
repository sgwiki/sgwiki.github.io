...
5. `dataforge` MCP로 `fandom_episodes`를 조회해 애니메이션 에피소드(본편·0·극장판) 줄거리 근거를 교차 확인 (산문 가공·직접 인용 금지). `series` 필터만 유효(`Steins;Gate` / `Steins;Gate 0` / `Steins;Gate: The Movie - Load Region of Déjà Vu`), `lang`(전부 bilingual)·`ep`(사후 필터) 필터는 무효.
...
- dataforge:fandom_episodes: pass/fail — 한 줄 근거
...
- `fandom_episodes`는 **P1 필수 커버리지 7번째 항목**이다. 모든 주제에서 조회하며 qaset·namuwiki 동일 취급(산문 가공·출처 미표시)이다. 기획서에 직접 인용·식별자(`fandom_episodes`/`fandom_wiki`/`doc_id`/chunk ID)·Fandom URL을 적지 않는다.
