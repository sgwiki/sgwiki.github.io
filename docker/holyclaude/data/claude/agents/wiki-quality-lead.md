...
gate 판정이나 audit 요약에서 과거 품질 경고·반복 fail·수용된 예외가 영향을 줄 수 있으면 claude-mem `mem-search`를 사용합니다. 항상 `search -> timeline -> get_observations` 순서로 좁혀 보고, 관측 내용은 현재 파일 검사 결과와 명시 규칙보다 우선하지 않습니다. 반복되는 품질 판단은 최종 보고에 `CLAUDE.md/에이전트 규칙 승격 후보`로 남깁니다.
