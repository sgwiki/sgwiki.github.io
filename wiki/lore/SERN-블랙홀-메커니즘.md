...
[타임리프](타임리프.md)의 핵심 기술은 인간의 단기 기억(약 3.2TB로 가정)을 마이크로 특이점에 통과시켜 **36바이트**까지 초압축하는 것이다. 작중 설정상 5단계를 거친다.[^3]
...
타임리프의 기본 한계는 48시간이다. 마키세 크리스는 "성격은 기억만으로 형성되는 것이 아니며, 발신자와 수신자의 뇌 구조가 가장 유사한 최근 버전의 자신으로 제한되어야 한다"는 논리로 한계를 설명한다. 히야죠 마호의 개량으로 SG0 시점에서는 2주까지 확장된다.
...
[타임리프](타임리프.md)는 §5의 5단계 메커니즘을 따른다. 데이터 압축 + 시간 역행이 동시에 발생하는 점이 D메일과의 본질적 차이다. 마호의 개량 전에는 48시간 한계, 이후에는 2주까지 확장.

### 6.3 타임머신(FG204 / C204)

타임머신은 사람이나 물리적 물체를 과거로 직접 이동시킨다. 존 타이타(John Titor)의 증언과 작중 기술은 **마이크로 특이점 2개를 회전시켜 Kerr 블랙홀을 형성**하고, 승객 캡슐이 그 벌거벗은 특이점을 통과하도록 설계된다.
...
[다이버전스 미터](다이버전스-미터.md)는 Kerr 블랙홀 이론에 기반한 중력 정현파를 측정하여 세계선 변동률을 수치화하는 장치로, 존 타이타의 기술을 미래의 오카베가 완성한 것이다. 이 역시 마이크로 특이점 기술의 응용이다.
...
- [존 타이타](존-티토와-존-타이타.md) — Kerr 블랙홀 타임머신 이론 증언자
...
[^3]: 나무위키 [「타임 리프(Steins;Gate)」](https://namu.wiki/w/%ED%83%80%EC%9E%84%20%EB%A6%AC%ED%94%84%28Steins%3BGate%29) — '타임 리프의 이론'(공식 자료집 88p 전재) 섹션 기반 서술
