...
팬 분석 측면에서는 "The Mechanics of Steins;Gate"가 세계선과 변동률 관측의 철학을 다음과 같이 정리한다.

> **[팬 분석]** "각 세계선은 '다이버전스'라 불리는 양으로 라벨링될 수 있다. 이는 세계 간에 변하는 중력의 특징을 정량화하는 객관적 척도이며, 그 영점은 최초의 다이버전스 미터가 구축된 세계선 위에 놓인다. 동등한 진술은, 세계선 변동률의 각 상수 값에 대해 세계선이 존재한다는 것이다. 이 관점은 정의상 세계선은 변동률이 변하는 사건을 포함할 수 없다는 것을 강조한다." — The Mechanics of Steins;Gate[^1]
