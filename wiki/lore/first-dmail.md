...
본 문서는 D메일이라는 기술의 일반론을 다루지 않는다. 메커니즘 전반은 [D메일](./dmail.md)을, 전화렌지(가칭)의 구조는 [전화렌지(폰웨이브)](./phonewave.md)를 참조. 본 문서는 '왜 이 한 통의 메일이 세계선을 뒤집었는가'라는 인과 사슬과 '이미 보내진 사건을 어떻게 되돌리는가'라는 영속성 문제에 집중한다.
...
오카베가 보낸 메시지가 '단순한 SMS'에서 'D메일'로 격상된 것은 전적으로 우발적이었다. 발신자 오카베도 수신자 다루도 이 메시지가 과거에 도달할 것이라 예상하지 않았다. 다만 다루가 전화렌지(가칭)의 실험을 12시~18시 사이에 반복하고 있었고, 그 시간대에 오카베의 SMS가 단말기에 수신되면서 우발적 방전 현상이 일어난 것이다. 이 사건이 계기가 되어 오카베와 다루, 크리스는 이후 의도적으로 D메일을 재현·활용하게 된다. 전화렌지(가칭)가 어떻게 메시지를 과거로 보내는지에 대한 구조적 해설은 [전화렌지(폰웨이브)](./phonewave.md)에서 다룬다.
...
오퍼레이션 베르단디의 상세한 절차와 IBN 5100 회수 경로는 [오퍼레이션 베르단디](./operation-verdandi.md)에서 다룬다. 베타 회귀 이후 크리스를 구하기 위한 최종 단계는 [알파 어트랙터 필드 탈출](./alpha-escape.md)과 [스쿨드 작전](./operation-skuld.md)을 참조.
...
- [D메일](./dmail.md) — D메일 기술 일반론, 메커니즘, 세계선 변동 원리
- [전화렌지(폰웨이브)](./phonewave.md) — D메일 송신 장치의 구조와 리프터 역할
- [로또 6 D메일](./lotto6-dmail.md) — 첫 D메일 직후 이루어진 두 번째 D메일 검증 실험
- [오퍼레이션 베르단디](./operation-verdandi.md) — 첫 D메일 에셜론 기록 삭제 작전
- [알파 어트랙터 필드 탈출](./alpha-escape.md) — 알파 → 베타 회귀 일반론
- [에셜론](./echelon.md) — SERN의 전 세계 감시망
- [IBN 5100](./ibn-5100.md) — 에셜론 DB 접근에 필요한 레거시 컴퓨터
- [SERN](./sern.md) — 타임머신 독점을 노리는 연구 기관
- [리딩 슈타이너](./reading-steiner.md) — 세계선 이동 시 오카베만 경험하는 기억 보존 현상
- [나카바치 논문](./nakabachi-paper.md) — 크리스 사망 사건의 직접적 배경
- [라디오 회관](./radio-kaikan.md) — 첫 D메일 사건의 무대
