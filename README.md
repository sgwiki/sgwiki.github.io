...
P5는 기존 `wiki/*.md` 페이지를 선정해 파일당 `wiki-restructurer`(구조) → `wiki-rewriter`(위키 고유 교정) → `wiki-humanizer`(AI-문체 제거) → `source-sanitizer` → `humanize_fact_guard` → `wiki-linker` → 팀장 diff → commit/push 순으로 정비합니다. 역할 분리 원칙에 따라 **`wiki-rewriter`는 VOCAB_GUIDE 용어 통일·내부 식별자 스크럽·사실/스포일러 보존 검토**만 맡고, **번역투·헤징·형식명사·기계적 리듬 같은 일반 AI-문체 교정은 `wiki-humanizer`가 [Humanize KR](https://github.com/epoko77-ai/im-not-ai) 플러그인(`/humanize --strict`)으로 전담**합니다(이중 편집 방지).

humanize는 문체만 손보고 사실·수치·인용·스포일러는 바꾸지 않으며, 이 불변식은 `scripts/humanize_fact_guard.py`(humanize 전/후 스냅샷의 숫자 토큰·인용 블록 텍스트·frontmatter `spoiler` 동일성)가 결정적으로 재검증합니다. 가드가 위반(exit 1)하면 팀장은 해당 파일 변경을 `git checkout`으로 되돌리고 registry release하며 **commit하지 않습니다**. 플러그인이 미설치·실행 실패이면 문체 제거 없이 파이프라인이 진행됩니다(부팅 비블로킹과 동일 원칙).
